...
- Keep automatic web research off by default. When enabled by the user, agents may send visible queries to a third-party search provider without per-query approval.
...
- Web-search requests send only the visible query to the search provider. By default this requires an Approve click; the user can enable meeting-level automatic web research to skip per-query approval. Results are added to the in-memory meeting research context.
